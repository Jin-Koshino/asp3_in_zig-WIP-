///
///  セマフォ機能のテスト(1)のシステムコンフィギュレーション記述
///
///  $Id$
///
...
fn configuration(comptime cfg: *CfgData) void {
    ...
    cfg.CRE_TSK("TASK5", CTSK(TA_NULL, 5, task5, MID_PRIORITY, STACK_SIZE, null));
    cfg.CRE_ALM("ALM1", CALM(TA_NULL, NFY_TMEHDR(1, alarm1_handler)));
    cfg.CRE_SEM("SEM1", CSEM(TA_NULL, 1, 1));
    cfg.CRE_SEM("SEM2", CSEM(TA_NULL, 2, 2));
    cfg.CRE_SEM("SEM3", CSEM(TA_TPRI, 0, 1));
}
