///
///  ミューテックスのテスト(4)のシステムコンフィギュレーション記述
///
///  $Id$
///
...
fn configuration(comptime cfg: *CfgData) void {
    ...
    cfg.CRE_TSK("TASK4", CTSK(TA_NULL, 4, task4, HIGH_PRIORITY, STACK_SIZE, null));
    cfg.CRE_TSK("TASK5", CTSK(TA_NULL, 5, task5, HIGH_PRIORITY, STACK_SIZE, null));
    cfg.CRE_MTX("MTX1", CMTX(TA_CEILING, MID_PRIORITY));
    cfg.CRE_MTX("MTX2", CMTX(TA_CEILING, HIGH_PRIORITY));
    cfg.CRE_MTX("MTX3", CMTX(TA_CEILING, HIGH_PRIORITY));
}
