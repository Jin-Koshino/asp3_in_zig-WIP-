...
fn configuration(comptime cfg: *CfgData) void {
    ...
    cfg.CRE_ALM("ALM1", CALM(TA_NULL, NFYINFO(.{ TNFY_SETVAR, p_event_variable, 1 }, cfg)));
    cfg.CRE_ALM("ALM2", CALM(TA_NULL, NFYINFO(.{ TNFY_ACTTSK, "TASK2" }, cfg)));
    cfg.CRE_ALM("ALM3", CALM(TA_NULL, NFYINFO(.{ TNFY_WUPTSK | TENFY_SETVAR, "TASK2", p_error_variable }, cfg)));
    cfg.CRE_ALM("ALM4", CALM(TA_NULL, NFYINFO(.{ TNFY_SIGSEM | TENFY_ACTTSK, "SEM1", "TASK2" }, cfg)));
    cfg.CRE_ALM("ALM5", CALM(TA_NULL, NFYINFO(.{ TNFY_SETFLG, "FLG1", 0x01 }, cfg)));
    cfg.CRE_ALM("ALM6", CALM(TA_NULL, NFYINFO(.{ TNFY_SNDDTQ | TENFY_WUPTSK, "DTQ1", 0x01, "TASK2" }, cfg)));
    cfg.CRE_ALM("ALM7", CALM(TA_NULL, NFYINFO(.{ TNFY_ACTTSK | TENFY_SIGSEM, "TASK2", "SEM1" }, cfg)));
    cfg.CRE_ALM("ALM8", CALM(TA_NULL, NFYINFO(.{ TNFY_ACTTSK | TENFY_SETFLG, "TASK2", "FLG1", 0x02 }, cfg)));
    cfg.CRE_ALM("ALM9", CALM(TA_NULL, NFYINFO(.{ TNFY_ACTTSK | TENFY_SNDDTQ, "TASK2", "DTQ1" }, cfg)));
    cfg.CRE_ALM("ALM10", CALM(TA_NULL, NFYINFO(.{ TNFY_INCVAR, p_count_variable }, cfg)));
    cfg.CRE_ALM("ALM11", CALM(TA_NULL, NFYINFO(.{ TNFY_ACTTSK | TENFY_INCVAR, "TASK2", p_count_variable }, cfg)));
}
