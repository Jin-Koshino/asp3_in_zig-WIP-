...
///
pub const t_stddef = @import("t_stddef.zig");

///
///  ターゲット依存部
///
///const target = @import("../target/" ++ option.TARGET ++ "/target_kernel.zig");
const target = @import("../target/ct11mpcore_gcc/target_kernel.zig");
pub const publish = target.publish;
////
const STK_T = target.STK_T;
const MPF_T = target.MPF_T;
////
...
pub const TOPPERS_SUPPORT_OVRHDR = // オーバランハンドラ機能拡張
    if (option.SUPPORT_OVRHDR) isTrue(target, "TOPPERS_SUPPORT_OVRHDR") else false;
...
pub const T_CTSK = struct {
    tskatr: t_stddef.ATR = t_stddef.TA_NULL, // タスク属性
    exinf: t_stddef.EXINF = t_stddef.castToExinf(0), // タスクの拡張情報
    task: TASK, // タスクのメインルーチンの先頭番地
    itskpri: t_stddef.PRI, // タスクの起動時優先度
    stksz: usize, // タスクのスタック領域のサイズ
    stk: ?[*]u8 = null, // タスクのスタック領域
};

pub const T_RTSK = extern struct {
    tskstat: t_stddef.STAT, // タスク状態
    tskpri: t_stddef.PRI, // タスクの現在優先度
    tskbpri: t_stddef.PRI, // タスクのベース優先度
    tskwait: t_stddef.STAT, // 待ち要因
    wobjid: t_stddef.ID, // 待ち対象のオブジェクトのID
    lefttmo: t_stddef.TMO, // タイムアウトするまでの時間
...
pub const T_CMTX = struct {
    mtxatr: t_stddef.ATR = t_stddef.TA_NULL, // ミューテックス属性
    ceilpri: t_stddef.PRI, // ミューテックスの上限優先度
};

pub const T_RMTX = extern struct {
    htskid: t_stddef.ID, // ミューテックスをロックしているタスクのID番号
    wtskid: t_stddef.ID, // ミューテックスの待ち行列の先頭のタスクのID番号
};

pub const T_CMPF = struct {
    mpfatr: t_stddef.ATR = t_stddef.TA_NULL, // 固定長メモリプール属性
...
pub const T_RMPF = extern struct {
    wtskid: t_stddef.ID, // 固定長メモリプールの待ち行列の先頭のタスクの
    // ID番号
...
pub const T_NFYINFO = struct {
    nfy: union(NfyMode) { // タイムイベントの通知に関する付随情報
        Handler: struct {
            exinf: t_stddef.EXINF = t_stddef.castToExinf(0),
            tmehdr: TMEHDR,
        },
        SetVar: struct {
            p_var: *usize,
...
pub const T_CCYC = struct {
    cycatr: t_stddef.ATR = t_stddef.TA_NULL, // 周期通知属性
    nfyinfo: T_NFYINFO, // 周期通知の通知方法
    cyctim: t_stddef.RELTIM, // 周期通知の通知周期
    cycphs: t_stddef.RELTIM, // 周期通知の通知位相
};

pub const T_RCYC = extern struct {
    cycstat: t_stddef.STAT, // 周期通知の動作状態
    lefttim: t_stddef.RELTIM, // 次回通知時刻までの相対時間
};

pub const T_CALM = struct {
    almatr: t_stddef.ATR = t_stddef.TA_NULL, // アラーム通知属性
    nfyinfo: T_NFYINFO, // アラーム通知の通知方法
};

pub const T_RALM = extern struct {
    almstat: t_stddef.STAT, // アラーム通知の動作状態
    lefttim: t_stddef.RELTIM, // 通知時刻までの相対時間
};

pub const T_DOVR = struct {
    ovratr: t_stddef.ATR = t_stddef.TA_NULL, // オーバランハンドラ属性
    ovrhdr: OVRHDR, // オーバランハンドラの先頭番地
};

pub const T_ROVR = extern struct {
    ovrstat: t_stddef.STAT, // オーバランハンドラの動作状態
    leftotm: t_stddef.PRCTIM, // 残りプロセッサ時間
};

pub const T_CINT = struct {
    intatr: t_stddef.ATR = t_stddef.TA_NULL, // 割込み要求ライン属性
...
pub const T_CISR = extern struct {
    ...
    isrpri: t_stddef.PRI = 0, // 割込みサービスルーチン優先度
};

pub const T_DINH = struct {
    inhatr: t_stddef.ATR = t_stddef.TA_NULL, // 割込みハンドラ属性
    inthdr: INTHDR, // 割込みハンドラの先頭番地
};

pub const T_DEXC = struct {
    excatr: t_stddef.ATR = t_stddef.TA_NULL, // CPU例外ハンドラ属性
...
pub const T_AINI = struct {
    iniatr: t_stddef.ATR = t_stddef.TA_NULL, // 初期化ルーチン属性
    exinf: t_stddef.EXINF = t_stddef.castToExinf(0), // 初期化ルーチンの拡張情報
    inirtn: INIRTN, // 初期化ルーチンの先頭番地
};

pub const T_ATER = struct {
    teratr: t_stddef.ATR = t_stddef.TA_NULL, // 終了処理ルーチン属性
    exinf: t_stddef.EXINF = t_stddef.castToExinf(0), // 終了処理ルーチンの拡張情報
...
///
pub const iact_tsk = publish.act_tsk;
pub const iwup_tsk = publish.wup_tsk;
pub const irel_wai = publish.rel_wai;
pub const isns_ter = publish.sns_ter;
pub const isig_sem = publish.sig_sem;
pub const iset_flg = publish.set_flg;
pub const ipsnd_dtq = publish.psnd_dtq;
pub const ifsnd_dtq = publish.fsnd_dtq;
pub const ipsnd_pdq = publish.psnd_pdq;
pub const ifch_hrt = publish.fch_hrt;
pub const ista_alm = publish.sta_alm;
pub const istp_alm = publish.stp_alm;
pub const ista_ovr = publish.sta_ovr;
pub const istp_ovr = publish.stp_ovr;
pub const irot_rdq = publish.rot_rdq;
pub const iget_tid = publish.get_tid;
pub const iloc_cpu = publish.loc_cpu;
pub const iunl_cpu = publish.unl_cpu;
pub const isns_ctx = publish.sns_ctx;
pub const isns_loc = publish.sns_loc;
pub const isns_dsp = publish.sns_dsp;
pub const isns_dpn = publish.sns_dpn;
pub const isns_ker = publish.sns_ker;
pub const iext_ker = publish.ext_ker;
pub const idis_int = publish.dis_int;
pub const iena_int = publish.ena_int;
pub const iclr_int = publish.clr_int;
pub const iras_int = publish.ras_int;
pub const iprb_int = publish.prb_int;
pub const ixsns_dpn = publish.xsns_dpn;
