...
pub fn set_cpsr(cpsr: u32) void {
    asm volatile ("msr cpsr_cxsf, %[cpsr]"
        :
        : [cpsr] "r" (cpsr),
        : "memory", "cc"
    );
...
pub fn CP15_READ_MIDR() u32 {
    return asm ("mrc p15, 0, %[reg], c0, c0, 0"
        : [reg] "=r" (-> u32),
    );
}
...
pub fn CP15_READ_MPIDR() u32 {
    if (comptime isEnabled(Feature.has_v6)) {
        return asm ("mrc p15, 0, %[reg], c0, c0, 5"
            : [reg] "=r" (-> u32),
        );
    } else @compileError("not supported.");
}
...
pub fn CP15_READ_CTR() u32 {
    return asm ("mrc p15, 0, %[reg], c0, c0, 1"
        : [reg] "=r" (-> u32),
    );
}
...
pub fn CP15_READ_CLIDR() u32 {
    if (comptime isEnabled(Feature.has_v7)) {
        return asm ("mrc p15, 1, %[reg], c0, c0, 1"
            : [reg] "=r" (-> u32),
        );
    } else @compileError("not supported.");
}
...
pub fn CP15_WRITE_CSSELR(reg: u32) void {
    if (comptime isEnabled(Feature.has_v7)) {
        asm volatile ("mcr p15, 2, %[reg], c0, c0, 0"
            :
            : [reg] "r" (reg),
        );
    } else @compileError("not supported.");
...
pub fn CP15_READ_CCSIDR() u32 {
    if (comptime isEnabled(Feature.has_v7)) {
        return asm ("mrc p15, 1, %[reg], c0, c0, 0"
            : [reg] "=r" (-> u32),
        );
    } else @compileError("not supported.");
}
...
pub fn CP15_READ_SCTLR() u32 {
    return asm ("mrc p15, 0, %[reg], c1, c0, 0"
        : [reg] "=r" (-> u32),
    );
}
pub fn CP15_WRITE_SCTLR(reg: u32) void {
    asm volatile ("mcr p15, 0, %[reg], c1, c0, 0"
        :
        : [reg] "r" (reg),
    );
}
...
pub fn CP15_READ_ACTLR() u32 {
    return asm ("mrc p15, 0, %[reg], c1, c0, 1"
        : [reg] "=r" (-> u32),
    );
}
pub fn CP15_WRITE_ACTLR(reg: u32) void {
    asm volatile ("mcr p15, 0, %[reg], c1, c0, 1"
        :
        : [reg] "r" (reg),
    );
}
...
pub fn CP15_READ_CPACR() u32 {
    return asm ("mrc p15, 0, %[reg], c1, c0, 2"
        : [reg] "=r" (-> u32),
    );
}
pub fn CP15_WRITE_CPACR(reg: u32) void {
    asm volatile ("mcr p15, 0, %[reg], c1, c0, 2"
        :
        : [reg] "r" (reg),
    );
}
...
pub fn CP15_INVALIDATE_ICACHE() void {
    asm volatile ("mcr p15, 0, %[reg], c7, c5, 0"
        :
        : [reg] "r" (@as(u32, 0)),
    );
}
...
pub fn CP15_INVALIDATE_BP() void {
    asm volatile ("mcr p15, 0, %[reg], c7, c5, 6"
        :
        : [reg] "r" (@as(u32, 0)),
    );
}
...
pub fn CP15_INVALIDATE_DCACHE() void {
    if (comptime !isEnabled(Feature.has_v7)) {
        asm volatile ("mcr p15, 0, %[reg], c7, c6, 0"
            :
            : [reg] "r" (@as(u32, 0)),
        );
    } else @compileError("not supported.");
...
pub fn CP15_INVALIDATE_UCACHE() void {
    if (comptime !isEnabled(Feature.has_v7)) {
        asm volatile ("mcr p15, 0, %[reg], c7, c7, 0"
            :
            : [reg] "r" (@as(u32, 0)),
        );
    } else @compileError("not supported.");
...
pub fn CP15_CLEAN_AND_INVALIDATE_DCACHE() void {
    if (comptime isEnabled(Feature.has_v6) and !isEnabled(Feature.has_v7)) {
        asm volatile ("mcr p15, 0, %[reg], c7, c14, 0"
            :
            : [reg] "r" (@as(u32, 0)),
        );
    } else @compileError("not supported.");
...
pub fn CP15_CLEAN_AND_INVALIDATE_UCACHE() void {
    if (comptime isEnabled(Feature.has_v6) and !isEnabled(Feature.has_v7)) {
        asm volatile ("mcr p15, 0, %[reg], c7, c15, 0"
            :
            : [reg] "r" (@as(u32, 0)),
        );
    } else @compileError("not supported.");
...
pub fn CP15_WRITE_DCISW(reg: u32) void {
    asm volatile ("mcr p15, 0, %[reg], c7, c6, 2"
        :
        : [reg] "r" (reg),
    );
}
...
pub fn CP15_WRITE_DCCISW(reg: u32) void {
    asm volatile ("mcr p15, 0, %[reg], c7, c14, 2"
        :
        : [reg] "r" (reg),
    );
}
...
pub fn CP15_READ_DFSR() u32 {
    if (comptime isEnabled(Feature.has_v6)) {
        return asm ("mrc p15, 0, %[reg], c5, c0, 0"
            : [reg] "=r" (-> u32),
        );
    } else @compileError("not supported.");
}
...
pub fn CP15_READ_DFAR() u32 {
    if (comptime isEnabled(Feature.has_v6)) {
        return asm ("mrc p15, 0, %[reg], c6, c0, 0"
            : [reg] "=r" (-> u32),
        );
    } else @compileError("not supported.");
}
...
pub fn CP15_READ_IFSR() u32 {
    if (comptime isEnabled(Feature.has_v6)) {
        return asm ("mrc p15, 0, %[reg], c5, c0, 1"
            : [reg] "=r" (-> u32),
        );
    } else @compileError("not supported.");
}
...
pub fn CP15_READ_IFAR() u32 {
    if (comptime isEnabled(Feature.has_v6)) {
        return asm ("mrc p15, 0, %[reg], c6, c0, 2"
            : [reg] "=r" (-> u32),
        );
    } else @compileError("not supported.");
}
...
pub fn CP15_WRITE_TTBCR(reg: u32) void {
    if (comptime isEnabled(Feature.has_v6)) {
        asm volatile ("mcr p15, 0, %[reg], c2, c0, 2"
            :
            : [reg] "r" (reg),
        );
    } else @compileError("not supported.");
...
pub fn CP15_READ_TTBR0() u32 {
    return asm ("mrc p15, 0, %[reg], c2, c0, 0"
        : [reg] "=r" (-> u32),
    );
}
pub fn CP15_WRITE_TTBR0(reg: u32) void {
    asm volatile ("mcr p15, 0, %[reg], c2, c0, 0"
        :
        : [reg] "r" (reg),
    );
}
...
pub fn CP15_WRITE_DACR(reg: u32) void {
    asm volatile ("mcr p15, 0, %[reg], c3, c0, 0"
        :
        : [reg] "r" (reg),
    );
}
...
pub fn CP15_WRITE_CONTEXTIDR(reg: u32) void {
    if (comptime isEnabled(Feature.has_v6)) {
        asm volatile ("mcr p15, 0, %[reg], c13, c0, 1"
            :
            : [reg] "r" (reg),
        );
    } else @compileError("not supported.");
...
pub fn CP15_INVALIDATE_TLB() void {
    asm volatile ("mcr p15, 0, %[reg], c8, c7, 0"
        :
        : [reg] "r" (@as(u32, 0)),
    );
}
...
pub fn CP15_READ_PMCR() u32 {
    if (comptime isEnabled(Feature.has_v7)) {
        return asm ("mrc p15, 0, %[reg], c9, c12, 0"
            : [reg] "=r" (-> u32),
        );
    } else @compileError("not supported.");
}
pub fn CP15_WRITE_PMCR(reg: u32) void {
    if (comptime isEnabled(Feature.has_v7)) {
        asm volatile ("mcr p15, 0, %[reg], c9, c12, 0"
            :
            : [reg] "r" (reg),
        );
    } else @compileError("not supported.");
...
pub fn CP15_READ_PMCNTENSET() u32 {
    if (comptime isEnabled(Feature.has_v7)) {
        return asm ("mrc p15, 0, %[reg], c9, c12, 1"
            : [reg] "=r" (-> u32),
        );
    } else @compileError("not supported.");
}
pub fn CP15_WRITE_PMCNTENSET(reg: u32) void {
    if (comptime isEnabled(Feature.has_v7)) {
        asm volatile ("mcr p15, 0, %[reg], c9, c12, 1"
            :
            : [reg] "r" (reg),
        );
    } else @compileError("not supported.");
...
pub fn CP15_READ_PMCCNTR() u32 {
    if (comptime isEnabled(Feature.has_v7)) {
        return asm ("mrc p15, 0, %[reg], c9, c13, 0"
            : [reg] "=r" (-> u32),
        );
    } else @compileError("not supported.");
}
pub fn CP15_WRITE_PMCCNTR(reg: u32) void {
    if (comptime isEnabled(Feature.has_v7)) {
        asm volatile ("mcr p15, 0, %[reg], c9, c13, 0"
            :
            : [reg] "r" (reg),
        );
    } else @compileError("not supported.");
...
pub fn CP15_READ_VBAR() u32 {
    if (comptime isEnabled(Feature.has_v7)) {
        return asm ("mrc p15, 0, %[reg], c12, c0, 0"
            : [reg] "=r" (-> u32),
        );
    } else @compileError("not supported.");
}
pub fn CP15_WRITE_VBAR(reg: u32) void {
    if (comptime isEnabled(Feature.has_v7)) {
        asm volatile ("mcr p15, 0, %[reg], c12, c0, 0"
            :
            : [reg] "r" (reg),
        );
    } else @compileError("not supported.");
...
test "isEnabled" {
    std.debug.assert(!isEnabled(Feature.has_v6) == true);
    std.debug.assert(!isEnabled(Feature.has_v7) == true);
}
